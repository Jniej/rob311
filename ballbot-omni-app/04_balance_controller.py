...
class ROB311BTController(Controller):
    ...
    def on_R2_press(self, value):
        ...
        self.tz_demo_1 = (1.0 + (value/JOYSTICK_SCALE))/1.0
    ...
    def on_L2_press(self, value):
        ...
        self.tz_demo_1 = -1 * (1.0 + (value/JOYSTICK_SCALE))/1.0
...
KP_THETA_X = 6.88                                  # Adjust until the system balances
KP_THETA_Y = 6.87                                  # Adjust until the system balances
KI_THETA_X = 0.07
KI_THETA_Y = 0.07
...
if __name__ == "__main__":
    ...
    imu_states = {'names': ['Roll (theta_x)', 'Pitch (theta_y)'],
                    'title': "Orientation",
                    'ylabel': "deg",
                    'xlabel': "time",
                    'colors' : ["r", "g"],
    ...
    motor_velocities = {'names': ['p_x', 'p_y', 'i_x'],
    ...
    controller_efforts = {'names': ['p_x', 'p_y', 'i_x', 'i_y'],
                    'title': "Controller Effort Due to Proportional and Integral Terms",
                    'ylabel': "Terms",
                    'xlabel': "time",
                    'colors' : ["r", "g", "b", "c"],
                    'line_width': [2]*4,
                    }    
    plot_config = [imu_states, controller_efforts]
    ...
    for t in SoftRealtimeLoop(dt=DT, report=True):
        ...
        #print("I error (x,y): ", error_int_x, error_int_y)
        i_term_x = KI_THETA_X * error_int_x #* DT
        i_term_y = KI_THETA_Y * error_int_y #* DT
        if np.abs(i_term_x) > 0.5 * MAX_PLANAR_DUTY:
            i_term_x = np.sign(i_term_x) * 0.5 * MAX_PLANAR_DUTY
        if np.abs(i_term_y) > 0.5 * MAX_PLANAR_DUTY:
            i_term_y = np.sign(i_term_y) * 0.5 * MAX_PLANAR_DUTY
        ...
        print("AAA: ",  rob311_bt_controller.tz_demo_1)
        ...
        print("Iteration no. {}, Tx: {:.2f}, Ty: {:.2f}, Tz: {:.2f}".format(i, Tx, Ty, Tz))
        ...
        rtplot_data = [(180 * theta_x) / np.pi, (180 * theta_y) / np.pi, (KP_THETA_X * error_x), (KP_THETA_Y * error_y), i_term_x, i_term_y]
